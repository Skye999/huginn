...
module Agents
  class ImapFolderAgent < Agent
    ...
    event_description <<-MD
      ...
      Additionally, "rfc822" will be included if the `include_rfc822` option is set.
    MD
    ...
    def validate_options
      ...
      %w[ssl mark_as_read include_rfc822].each { |key|
    ...
    def check
      each_unread_mail { |mail, notified|
        ...
        if notified.include?(mail.message_id)
          ...
          if boolify(interpolated['include_rfc822'])
            payload['rfc822'] = mail.rfc822
          end
    ...
    class Message < SimpleDelegator
      ...
      def fetch
        @parsed ||= Mail.read_from_string(rfc822)
      end
